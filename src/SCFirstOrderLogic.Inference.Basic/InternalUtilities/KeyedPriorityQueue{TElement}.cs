...
internal sealed class KeyedPriorityQueue<TElement, TPriority>
{
    ...
    ////{
    ////    if (heapIndex < 0 || heapIndex >= Count)
    ////    {
    ////        throw new IndexOutOfRangeException();
    ////    }

    ////    var element = heap[--Count];
    ////    var comparison = priorityComparison(heap[heapIndex], element);

    ////    if (comparison < 0)
    ////    {
    ////        BubbleUp(heapIndex, element);
    ////    }
    ////    else if (comparison > 0)
    ////    {
    ////        BubbleDown(heapIndex, element);
    ////    }
    ////    else
    ////    {
    ////        heap[heapIndex] = element;
    ////        indicesByElement[element] = heapIndex;
    ////    }
